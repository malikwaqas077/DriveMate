...
class _StudentLessonsScreenState extends State<StudentLessonsScreen> {
  ...
  Future<void> _showCancellationDialog(BuildContext context, Lesson lesson) async {
    // Calculate charge percentage based on instructor's policy
    final policy = _instructor?.cancellationPolicy;
    final windowHours = policy?.windowHours ?? 24;
    final chargePercent = policy?.chargePercent ?? 50;

    final hoursUntilLesson = lesson.startAt.difference(DateTime.now()).inHours;
    final isWithinWindow = hoursUntilLesson < windowHours;
    final effectiveCharge = isWithinWindow ? chargePercent : 0;
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      isWithinWindow
                                          ...
                                          : 'No charges apply - lesson is more than $windowHours hours away',
                                      style: TextStyle(
                                        fontSize: 13,
                                        color: isWithinWindow
                                            ? AppTheme.neutral700
